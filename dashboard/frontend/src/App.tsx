...
type Tab = "overview" | "analytics" | "strategies" | "pairs" | "risk" | "news" | "agent";
...
export default function App() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // agent is 7th parallel result

        if (portfolio?.risk) {
          setRiskForm({
            max_daily_loss_usdc: portfolio.risk.max_daily_loss_usdc ?? 150,
            max_position_pct_equity: portfolio.risk.max_position_pct_equity ?? 0.08,
            max_leverage: portfolio.risk.max_leverage ?? 3,
            max_open_positions: portfolio.risk.max_open_positions ?? 6,
          });
        }
      } catch (e) {
        console.error(e);
  ...
  const saveRiskSettings = async () => {
    ...
    try {
      const res = await apiFetch(`/api/risk`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          max_daily_loss_usdc: Number(riskForm.max_daily_loss_usdc),
          max_position_pct_equity: Number(riskForm.max_position_pct_equity),
          max_leverage: Number(riskForm.max_leverage),
          max_open_positions: Number(riskForm.max_open_positions),
        }),
      });
      const data = await res.json();
      if (data.ok || data.risk) {
        setState((prev) =>
          prev ? { ...prev, risk: { ...prev.risk, ...data.risk } } : prev
        );
        setRiskMsg("Settings saved");
      } else {
        setRiskMsg("Save failed");
      }
    } catch (e) {
      setRiskMsg("Network error");
    } finally {
      setSavingRisk(false);
  ...
  const tabs: { id: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "agent", label: "Agent", icon: <Brain className="w-4 h-4" /> },
  ];
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-400">
                  Emergency stop: blocks ALL NEW orders. Open positions stay open (not force-closed). Use if the bot misbehaves or losses spike. Reset only when safe.
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
              ...
              <p className="text-sm text-slate-400 mb-5">
                Change limits and save. Applied to the live risk manager state.
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label className="block">
                  <span className="text-xs text-slate-400">Max Daily Loss (USDC)</span>
                  <input
                    type="number"
                    step="1"
                    min="1"
                    value={riskForm.max_daily_loss_usdc}
                    onChange={(e) =>
                      setRiskForm((f) => ({
                        ...f,
                        max_daily_loss_usdc: Number(e.target.value),
                      }))
                    }
                    className="mt-1 w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 focus:outline-none focus:border-emerald-500"
                  />
                </label>

                <label className="block">
                  <span className="text-xs text-slate-400">Max Position % of Equity</span>
                  <input
                    type="number"
                    step="0.01"
                    min="0.01"
                    max="1"
                    value={riskForm.max_position_pct_equity}
                    onChange={(e) =>
                      setRiskForm((f) => ({
                        ...f,
                        max_position_pct_equity: Number(e.target.value),
                      }))
                    }
                    className="mt-1 w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 focus:outline-none focus:border-emerald-500"
                  />
                  <span className="text-xs text-slate-500">e.g. 0.08 = 8%</span>
                </label>

                <label className="block">
                  <span className="text-xs text-slate-400">Max Leverage</span>
                  <input
                    type="number"
                    step="0.5"
                    min="1"
                    ...
                    onChange={(e) =>
                      setRiskForm((f) => ({
                        ...f,
                        max_leverage: Number(e.target.value),
                      }))
                    }
                    className="mt-1 w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 focus:outline-none focus:border-emerald-500"
                  />
              ...
              <div className="mt-5 flex items-center gap-3">
                <button
                  ...
                  disabled={savingRisk}
                  className="px-5 py-2.5 rounded-lg font-medium bg-emerald-600 hover:bg-emerald-500 text-white disabled:opacity-50"
                >
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-400">
                  Blocks all NEW orders. Open positions stay open until strategy exits or you close them. Does not flatten the book automatically.
                </p>
              </div>
              <button
                onClick={toggleKillSwitch}
          ...
          <div className="space-y-4">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
              ...
              <p className="text-sm text-slate-400 mb-4">
                Session: <span className="text-slate-200">{agentState?.session || "—"}</span>
                {" · "}
                Avg setup: <span className="text-slate-200">{agentState?.smart?.avg_setup_score?.toFixed?.(1) ?? agentState?.avg_setup_score ?? "—"}</span>
                {" · "}
                Adaptive mult: <span className="text-slate-200">{agentState?.adaptive_mult_hint ?? "—"}</span>
              </p>
...
{JSON.stringify(agentState?.health || {}, null, 2)}
                  </pre>
                </div>
                <div className="bg-slate-950/50 rounded-lg p-3">
                  <div className="text-xs text-slate-400 mb-2">Scoreboard (auto enable/disable)</div>
                  <pre className="text-xs text-slate-300 overflow-auto max-h-40">
{JSON.stringify(agentState?.scoreboard || agentState?.smart?.scoreboard || {}, null, 2)}
                  </pre>
                </div>
              </div>
            </div>
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
              <h3 className="font-medium mb-2">Regimes</h3>
              <pre className="text-xs text-slate-300 overflow-auto max-h-56">
{JSON.stringify(agentState?.regimes || agentState?.smart?.regimes || {}, null, 2)}
              </pre>
            </div>
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
              <h3 className="font-medium mb-2">Last backtest</h3>
              <p className="text-xs text-slate-500 mb-2">
                Run on VPS: python scripts/run_backtest.py (stop engine first to avoid 429)
              </p>
              <pre className="text-xs text-slate-300 overflow-auto max-h-48">
{JSON.stringify(agentState?.last_backtest || { message: "No backtest cache yet" }, null, 2)}
              </pre>
            </div>
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
              <h3 className="font-medium mb-2">Memory summary</h3>
              <pre className="text-xs text-slate-300 overflow-auto max-h-40">
{JSON.stringify(agentState?.memory || {}, null, 2)}
              </pre>
            </div>
          </div>
        )}
